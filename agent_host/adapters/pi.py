...
class PiStartupUnavailable(RuntimeError):
    def __init__(self, reason: str):
        self.availability = {"provider_id": "pi", "ready": False,
            ...
            "diagnostic": "Install the pinned runtime with npm ci --ignore-scripts in agent_host/pi_runtime; configure Pi authentication."}
...
class PiAdapter:
    ...
    def require_startup_ready(self):
        ...
        self._startup_readiness = {"authentication": "unknown", "version": PI_VERSION}
    ...
    def _command(self, request):
        command = [self.node_path, str(RUNTIME_DIR / "node_modules/@earendil-works/pi-coding-agent/dist/cli.js"),
    ...
    async def run(self, request, run_id, emit):
        ...
        env = {**os.environ, "PI_CODING_AGENT_DIR": str(self.agent_dir),
            ...
            "AMADEUS_PI_WORKSPACE": str(cwd), "AMADEUS_PI_ACCESS": access}
        env["AMADEUS_PI_PYTHON"] = sys.executable
        env["AMADEUS_PI_WEB_HELPER"] = str(RUNTIME_DIR.parent / "pi_web_tools.py")
        env["PI_WEB_TOOLS_CONFIG"] = str(self.agent_dir / "web-tools.json")
