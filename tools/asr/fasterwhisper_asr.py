...
def execute_asr(input_folder, output_folder, model_size, language, precision):
    if '-local' in model_size:
        ...
        model_path = f'assets/models/asr/faster-whisper-{model_size}'
    else:
        model_path = model_size
    if language == 'auto':
        language = None  # Let the model auto-detect the most likely language.
    ...
    for file_name in tqdm(input_file_names):
        try:
            ...
            if info.language == "zh":
                print("Detected Chinese text; forwarding to FunASR")
                if("only_asr" not in globals()):
                    from tools.asr.funasr_asr import only_asr  # Avoid loading FunASR unless needed.
