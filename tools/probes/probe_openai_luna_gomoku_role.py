r"""Measure an OpenAI GPT-5.6 model on the standard Gomoku role probe.

This product-inert probe reuses the fixtures, renderer, and deterministic
scorer from ``probe_gemini37_gomoku_role.py``. It sends one Responses API call
per case with an explicit reasoning effort and never invokes an AUIP action.
"""
...
async def _run_one(
    ...
    *,
    ...
    try:
        request = {
            "model": model,
            "instructions": common.ROLE_SYSTEM,
            "input": [{"role": "user", "content": content}],
            "reasoning": {"effort": reasoning_effort},
            "max_output_tokens": 700,
            "text_format": common.GomokuRoleDecision,
            "store": False,
        }
        ...
        if not isinstance(decision, common.GomokuRoleDecision):
            decision = common.GomokuRoleDecision.model_validate_json(
...
async def run_probe(args: argparse.Namespace) -> dict:
    ...
    await client.close()
    return {
        "schema": "amadeus.openai-gpt56-gomoku-role-probe.v1",
        "created_at": datetime.now(timezone.utc).isoformat(),
        "model": args.model,
        "thinking_mode": f"reasoning_effort_{args.reasoning_effort}",
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--output-dir",
        default=str(
            ROOT / "runtime" / "e2e_reports" / "openai_luna_gomoku_role"
        ),
    )
    return parser
...
def main() -> None:
    ...
    output_path = output_dir / f"{model_slug}_gomoku_role_{stamp}.json"
