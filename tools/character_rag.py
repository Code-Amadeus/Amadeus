...
def main() -> None:
    ...
    build.add_argument("--source", type=Path, required=True, help="JSON array of short reference texts")
    ...
    search.add_argument("--max-distance", type=float, default=0.33)
    for command in (build, search):
        command.add_argument("--index-dir", default=".amadeus/character-rag")
    ...
    if args.command == "build":
        ...
        result = {
            "query": args.query,
            "hits": index.search(args.query, top_k=args.top_k, max_distance=args.max_distance),
