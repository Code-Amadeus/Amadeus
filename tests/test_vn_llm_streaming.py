...
from vn_player.llm_client import VNLLMClient, _speech_header
...
def test_header_delivers_before_tail_and_reuses_session_client(monkeypatch):
    async def run():
        ...
        assert metrics["first_token_at_ms"] <= metrics["speech_ready_at_ms"] <= metrics["completed_at_ms"]
...
@pytest.mark.parametrize("raw", [
    HEADER[:-1],  # No delimiter after the speak object yet.
...
def test_incomplete_or_invalid_headers_cannot_authorize_speech(raw):
    assert _speech_header(raw) is None
...
def test_json_string_escapes_and_fences_are_parsed_as_data():
    ...
    for position in range(len(prefix) - 1):
        assert _speech_header(prefix[:position]) is None
    assert _speech_header('```json\n' + prefix)["speak"]["text"] == text
