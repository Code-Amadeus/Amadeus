...
- Only sibling leaf tasks form a pile. Compact parents remain separate connection endpoints. Up to seven frosted edges communicate stack depth; larger piles remain accessible through selection and project expansion. The [September 12 update](companion_project_overview.md) replaces the original spread pages with one complete project map. Manual front-card choice and attention priority share one ordering rule.
