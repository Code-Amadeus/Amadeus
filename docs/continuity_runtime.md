...
Time-aware filtering supports bounded absolute/relative dates, and high-fidelity excerpts are rendered as quoted transcript data under the existing Continuity context budget; selected turns quote both the user statement and a bounded, explicitly non-authoritative past-assistant excerpt. Topics the user muted ("不要再提…") are skipped unless the current turn raises them itself. The Host exposes content-free recent retrieval traces through `continuity.retrieval.trace`.
