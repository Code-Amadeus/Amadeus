...
"""
custom:
  bert_base_path: assets/models/gpt-sovits/pretrained/chinese-roberta-wwm-ext-large
  cnhuhbert_base_path: assets/models/gpt-sovits/pretrained/chinese-hubert-base
  device: cpu
  is_half: false
  t2s_weights_path: assets/models/gpt-sovits/pretrained/gsv-v2final-pretrained/s1bert25hz-5kh-longer-epoch=12-step=369668.ckpt
  vits_weights_path: assets/models/gpt-sovits/pretrained/gsv-v2final-pretrained/s2G2333k.pth
  version: v2
default:
  bert_base_path: assets/models/gpt-sovits/pretrained/chinese-roberta-wwm-ext-large
  cnhuhbert_base_path: assets/models/gpt-sovits/pretrained/chinese-hubert-base
  device: cpu
  is_half: false
  t2s_weights_path: assets/models/gpt-sovits/pretrained/s1bert25hz-2kh-longer-epoch=68e-step=50232.ckpt
  vits_weights_path: assets/models/gpt-sovits/pretrained/s2G488k.pth
  version: v1
default_v2:
  bert_base_path: assets/models/gpt-sovits/pretrained/chinese-roberta-wwm-ext-large
  cnhuhbert_base_path: assets/models/gpt-sovits/pretrained/chinese-hubert-base
...
default_v3:
  ...
  device: cpu
  is_half: false
  t2s_weights_path: assets/models/gpt-sovits/pretrained/s1v3.ckpt
  vits_weights_path: assets/models/gpt-sovits/pretrained/s2Gv3.pth
  version: v3
"""
...
class TTS:
    ...
    def init_bigvgan(self):
        if self.bigvgan_model is not None:
            return
        self.bigvgan_model = BigVGAN.from_pretrained(os.path.join(now_dir, "assets", "models", "gpt-sovits", "pretrained", "models--nvidia--bigvgan_v2_24khz_100band_256x"), use_cuda_kernel=False)  # if True, RuntimeError: Ninja is required to load C++ extensions
    ...
    def init_sr_model(self):
        if self.sr_model is not None:
            return
        try:
            self.sr_model = create_ap_bwe(self.configs.device, DictToAttrRecursive)
            self.sr_model_not_exist = False
        except (FileNotFoundError, APBWEUnavailable):
    ...
    def run(self, inputs:dict):
            ...
            inputs (dict):
                {
                    ...
                    "sample_steps": 32,           # int. sampling steps for subsequent sentences.
                    "first_sample_steps": 16,      # int. sampling steps for the first sentence only (faster TTFP).
        ...
        first_sample_steps = inputs.get("first_sample_steps", 16)
        ...
        try:
            ...
            is_first_item = True
            for item in data:
                current_sample_steps = first_sample_steps if is_first_item else sample_steps
                ...
                if not self.configs.is_v3_synthesizer:
                    ...
                    if parallel_infer:
                        ...
                        audio_fragments = self.v3_synthesis_batched_infer(
                                                                        ...
                                                                        batch_phones, 
                                                                        speed=speed_factor, 
                                                                        sample_steps=current_sample_steps
                                                                    )
                        ...
                        for i, idx in enumerate(tqdm(idx_list)):
                            ...
                            audio_fragment = self.v3_synthesis(
                                    _pred_semantic, phones, speed=speed_factor, sample_steps=current_sample_steps
                                )
                            batch_audio_fragment.append(
                                audio_fragment
                            )
