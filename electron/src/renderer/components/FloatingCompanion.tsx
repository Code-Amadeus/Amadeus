...
export default function FloatingCompanion() {
  ...
  const syncHitRegions = useCallback(() => {
    ...
    const regions = [...root.querySelectorAll<HTMLElement>('[data-companion-hit]')]
      ...
      .map(element => {
        ...
        const clips = [rootBounds, ...['.companion-constellation', '.companion-focus-panel']
  ...
  return (
    <div ref={rootRef} className={`floating-companion skin-${skin}`}>
      ...
      <button
        type="button"
        className="floating-companion-character-hit"
        data-companion-hit
        aria-label="显示或隐藏人物控制"
      ...
      <CompanionTaskConstellation tasks={tasks} fading={fading} faults={faults} acknowledged={acknowledged} speakingKey={speakingKey}
