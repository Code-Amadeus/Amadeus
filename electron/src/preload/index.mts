...
contextBridge.exposeInMainWorld('amadeus', {
  ...
  openElectronSlice: (bridge: { assetPort: number; bridgePort: number; assetVersion?: string; renderMaxFps: number; sliceBounds?: { x: number; y: number; width: number; height: number } }): Promise<boolean> => ipcRenderer.invoke('electron-slice.open', bridge),
