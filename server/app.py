...
async def bootstrap(port: int = 17777) -> None:
    ...
    # Continuity is Host-owned durable state.  C1 subscribes only to accepted
    # chat lifecycle events; it does not enter the Main Chat prompt or gain
    # Provider/AUIP authority.
    from core.continuity import ContinuityService

    continuity_service = ContinuityService.from_project_root(ROOT)
    continuity_service.start()
    continuity_service.bind_event_bus(bus)
    # C3 gives Main Chat only a bounded read-only projection. The Continuity
    # service remains the Host owner of durable state and has no execution
    # authority.
    from core.chat_runtime import get_chat_runtime as _get_continuity_chat_runtime

    _get_continuity_chat_runtime().configure(
        continuity_grounding_provider=continuity_service.grounding_for_turn
    )
    ...
    sys_h = SystemHandler()
    continuity_h = ContinuityHandler(continuity_service)
    ...
    for h in (chat_h, session_h, tts_h, asr_h, wake_h, vts_h, expr_h, sys_h, continuity_h, render_h, wallpaper_h, provider_h, capability_h, mcp_connection_h, provider_activity_h, work_h, work_preview_h, attention_h, auip_h, vn_h, vn_launch_h):
    ...
    def _barge_in_enabled() -> bool:
        # In click-to-talk Manual mode (Wake disabled), no background speech
        # detector may create another user turn.  Barge-in is enabled only for
        # hands-free modes: Wake or an active continuous Auto Mic session.
        return bool(
            AEC_REALTIME_ENABLED
            and AEC_REALTIME_BARGE_IN
            and (
                WAKE_ENABLED
                or asr_h.is_continuous_chat_mic()
            )
        )
    ...
    async def _interrupt_for_barge_in() -> None:
        ...
        try:
            # If an ASR session is already active (for example Auto Mic waiting
            # for the assistant), notify_turn_complete above releases that same
            # session.  Do not replace it with a fake Wake session.
            if asr_h.active:
                logger.info(
                    "barge-in reusing active ASR session source=%s",
                    asr_h.source,
                )
                return
            ...
            if not WAKE_ENABLED:
                logger.info(
                    "barge-in interrupted presentation without ASR rearm "
                    "(Wake disabled; Manual Mic remains click-to-talk)"
                )
                return
    ...
    async def _start_asr_from_wake(payload=None):
        payload = payload or {}
        if not WAKE_ENABLED:
            logger.info("wake callback ignored because WAKE_ENABLED is false")
            return
        qwen_hot_window = max(1.0, float(WAKE_AWAKE_SECONDS))
    ...
    async def _send_asr_text_to_chat(
            text: str,
            *,
            source: str,
            chat_source: str,
        ...
        try:
            ...
            logger.exception(
                "failed to check ASR prompt leak"
            )

        try:
            from server.speculative_turn import (
                get_speculative_launcher,
            )

            if await get_speculative_launcher().resolve(text):
                logger.info(
                    "%s text matched speculative turn; "
                    "confirmed, skip resend",
                    source,
                )
                # A pending speculative chat turn already exists, so Auto Mic
                # must still wait for that turn to finish before listening again.
                return True
        except Exception:
            logger.exception(
                "speculative resolve failed; sending normally"
            )

        import llm.client as _lcm

        provider = str(
            getattr(_lcm, "LLM_PROVIDER", "")
            or LLM_PROVIDER
        )
        ...
        try:
            ...
            logger.exception("%s auto-send to chat failed", source)
            return False
    ...
    async def _handle_asr_recognized(
            payload: dict,
    ) -> None:
        source = str(
        ...
        if source == "vn_player":
            await _handle_vn_player_asr_recognized(
                payload
            )
            return
        ...
        if source == "chat_mic":
            ...
            mode = str(
                source_payload.get("mode") or ""
            ).strip().lower()
            auto_send = (
                mode == "auto"
                or source_payload.get("auto_send") is True
            )

            if not auto_send:
                return

            sent = await _send_asr_text_to_chat(
                str(payload.get("text") or ""),
                source="Auto Mic ASR",
                chat_source="asr",
                enabled=True,
            ...
            # Auto Mic marks itself waiting before this callback.  If the text
            # was intentionally dropped or chat submission failed, release the
            # wait immediately instead of stalling until the safety timeout.
            if not sent and asr_h.is_waiting_turn_complete():
                await asr_h.notify_turn_complete(
                    "auto_send_skipped"
                )
    ...
    async def _handle_voice_chat_finished(payload: dict) -> None:
        ...
        if status in {"error", "empty"}:
            logger.info(
                "voice chat finished without playable turn (%s); resuming ASR loop",
                status,
            )
        ...
        # With TTS disabled there is no playback-complete callback to release
        # Auto Mic/Wake, so release the wait directly.
        if e2e_no_tts or TTS_BACKEND == "disabled":
            await asr_h.notify_turn_complete("chat_complete_no_tts")
            return
    ...
    async def _release_asr_after_playback_idle() -> None:
        ...
        while time.monotonic() < deadline:
            ...
            if (not player_busy) and playback_ready and pending_empty:
                logger.info("voice chat complete and playback idle; releasing ASR turn wait fallback")
                await asr_h.notify_turn_complete("chat_complete_idle")
                return
            await asyncio.sleep(0.2)
        if asr_h.is_waiting_turn_complete():
            logger.warning("voice chat complete fallback timed out; releasing ASR turn wait")
    ...
    async def _handle_asr_ready_to_listen(payload: dict) -> None:
        # Wake is an independent feature.  When disabled, Manual/Auto Mic must
        # never call into WakeService or perform Wake-specific microphone handoff.
        if not WAKE_ENABLED:
            return

        source = str(payload.get("source") or "")
        if source not in {"wake", "chat_mic", "barge_in"}:
            return

        logger.info("%s ASR ready; pausing wake service", source)
        try:
            await wake_h.stop({"close_shared_mic": False})
        except Exception:
            logger.exception("failed to pause wake service for %s", source)
    ...
    async def _handle_asr_listening_stopped(payload: dict) -> None:
        ...
        if reason in {"preempted_by_chat_mic", "chat_mic_mode_switch"}:
            # Another chat_mic session is about to take ownership; restarting
            # WakeService for a few milliseconds only creates a microphone race.
            return

        logger.info(
            "%s ASR ended (%s); restoring wake service",
            source,
            reason,
        )
        try:
        ...
        if not await _vn_runtime_active():
            await wake_h.start({})
    ...
    chat_h.configure(
        ...
        pending_sentence_items=pending_sentence_items,
        on_turn_finished=_handle_voice_chat_finished,
    ...
    try:
        ...
        work_status_narrator = None
        await continuity_service.aclose(graceful=True)
...
def _rebase_web_goal_for_selected_provider(
    ...
    *,
    ...
    if not (
        requested == "browser"
        and str(selected_provider or "").strip().lower() in {"research", "openclaw"}
