...
def render_provider_routing_addon(
    ...
    *,
    ...
    if "research" in providers:
        lines.append(
            wording(
                "- Research (provider=\"research\") is the native default for general Web lookup and open-ended research when no live Browser page must be preserved: source discovery, current-fact lookup, multi-source comparison, synthesis, and cited research reports. It runs independently of OpenClaw. Use Browser only when live page state must be retained/manipulated.",
                "- Research（provider=\"research\"）は、live Browser page を保持する必要がない一般 Web 検索・調査の native 既定 provider である。情報源探索、最新事実の確認、複数ソース比較、統合、引用付き調査報告に使い、OpenClaw には依存しない。live page state の保持・操作が必要な時だけ Browser を使う。",
            )
        )
    if "openclaw" in providers:
        lines.append(
            wording(
                "- OpenClaw remains available for open-ended non-code external/desktop work. When Research is registered, do not choose OpenClaw merely for Web lookup, source discovery, comparison, synthesis, or a research report; choose provider=\"research\" instead unless the user explicitly forces OpenClaw.",
                "- OpenClaw は open-ended な非コード外部作業・デスクトップ作業に引き続き利用できる。Research が登録されている場合、Web 検索、情報源探索、比較、統合、調査報告だけを理由に OpenClaw を選ばず、ユーザーが明示的に OpenClaw を強制した場合を除き provider=\"research\" を選ぶ。",
            )
        )
